...
async function initDatabase() {
  try {
    ...
    const {
      ...
      Bill,
      BillItem
    } = (0, _models.initModels)(sequelize);

    // Sync all models (create tables if they do not exist)
    await sequelize.sync({
      alter: true
    }); // alter updates schema if changed without dropping
    ...
    if (userCount === 0) {
      ...
      const hashedPassword = await _bcryptjs.default.hash(defaultPassword, 10);
      await User.create({
        username: 'admin',
    ...
    if (productCount === 0) {
      ...
      const sampleProducts = [{
        ...
        name: 'REAR SHOCKHOSPER',
        stock_qty: 5,
        selling_price: 4200.00
      }, {
        name: 'SHOCKHOSPER HARM',
        stock_qty: 8,
        selling_price: 1850.00
      }, {
        name: 'DROP LINK SUB ASSEMBLY',
        stock_qty: 6,
        selling_price: 1253.00
      }, {
        name: 'COURIER CHARGE',
        stock_qty: 999,
    ...
    if (customerCount === 0) {
      ...
      const items = [{
        ...
        bill_id: bill.id,
        s_no: 3,
        product_name: 'FRONT DISC PAD',
        qty: 1,
        amount: 250.00
      }, {
        bill_id: bill.id,
        s_no: 4,
        product_name: 'GRIP SET',
        qty: 1,
        amount: 140.00
      }, {
        bill_id: bill.id,
        s_no: 5,
        product_name: 'REAR DISC PAD',
        qty: 1,
        amount: 250.00
      }, {
        bill_id: bill.id,
        s_no: 6,
        product_name: 'REAR SHOCKHOSPER',
        qty: 1,
        amount: 4200.00
      }, {
        bill_id: bill.id,
        s_no: 7,
        product_name: 'SHOCKHOSPER HARM',
        qty: 1,
        amount: 1850.00
      }, {
        bill_id: bill.id,
        s_no: 8,
        product_name: 'DROP LINK SUB ASSEMBLY',
        qty: 1,
        amount: 1253.00
      }, {
        bill_id: bill.id,
        s_no: 9,
        product_name: 'COURIER CHARGE',
        qty: 1,
        amount: 350.00
      }, {
        bill_id: bill.id,
        s_no: 10,
        product_name: 'WATER WASH',
        qty: 1,
        amount: 250.00
      }, {
        bill_id: bill.id,
        s_no: 11,
        product_name: 'DISC OIL',
        qty: 1,
        amount: 120.00
      }, {
        bill_id: bill.id,
